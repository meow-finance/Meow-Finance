...
contract mMeowToken is ERC20("mMeow", "mMEOW"), Ownable {
  ...
  uint256 public depositTime = 1 hours;
  ...
  function setParams(uint256 _depositTime, uint256 _lockTime) public onlyOwner {
  ...
  function deposit(address _for, uint256 _amount) public {
    require(block.timestamp < endDeposit, "not within the specified period");
    uint256 totalMeow = meow.balanceOf(address(this));
    uint256 totalShares = totalSupply();
    require(_amount <= meow.balanceOf(msg.sender), "deposit:: insufficient amount");
    require(meow.transferFrom(msg.sender, address(this), _amount), "deposit:: transfer error");
  ...
  function withdraw(uint256 _share) public {
    require(block.timestamp > startWithdraw, "not within the specified period");
    uint256 totalShares = totalSupply();
