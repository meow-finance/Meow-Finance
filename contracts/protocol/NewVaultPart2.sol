...
contract NewVaultPart2 is ReentrancyGuardUpgradeSafe, OwnableUpgradeSafe {
  ...
  function setVault1(address _vaultPart1) public onlyOwner nonReentrant {
  ...
  function getmMeowFee(uint256 id, bytes calldata data) public view returns (uint256) {
    ...
    (, , uint256 mMeowFeeAmt) = mMeowFee.calMMeowFee(
      IWorker(pos.worker).baseToken(),
      (HEALTH_AFTER.sub(HEALTH_BEFORE)),
      pos.owner
    );
    return mMeowFeeAmt;
  }
