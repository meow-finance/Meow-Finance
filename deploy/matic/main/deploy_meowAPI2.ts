...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    await deploy('MeowAPI2', {
        from: deployer,
        args: [FairLaunch, SushiswapRouter, MiniChef, QuickswapRouter, StakingRewardsFactory, MeowToken, UsdcToken],
        log: true,
        deterministicDeployment: false,
    });

    const meowAPI2 = await deployments.get('MeowAPI2');
    console.log("MeowAPI2: ", meowAPI2.address);

    await hre.run("verify:verify", {
        address: meowAPI2.address,
        constructorArguments: [
            FairLaunch, SushiswapRouter, MiniChef, QuickswapRouter, StakingRewardsFactory, MeowToken, UsdcToken
        ],
    })
