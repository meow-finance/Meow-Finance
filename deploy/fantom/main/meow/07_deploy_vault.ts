...
import { checkIsVerified, WriteLogs, IsFork, Wait } from '../../../../global/function';
...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  for (let i = 0; i < VAULTS.length; i++) {
    ...
    if (!(await checkIsVerified(implAddr_debt))) {
      ...
      console.log(`${VAULTS[i].VAULT_NAME} DebtToken impl is verified.`);
    }
    if (!IsFork()) {
    ...
    if (!(await checkIsVerified(implAddr_vault))) {
      ...
      console.log(`${VAULTS[i].VAULT_NAME} Vault impl is verified.`);
    }
    if (!IsFork()) {
    ...
    await debtToken.setOkHolders([vault.address, MEOW_MINING_ADDR.address], true, { gasLimit: '2000000' });
    Wait(50000);
    console.log("✅ Done");

    console.log(`Transferring ownership of ${VAULTS[i].VAULT_NAME} debtToken to ${VAULTS[i].VAULT_NAME} Vault`);
    await debtToken.transferOwnership(vault.address, { gasLimit: '2000000' });
    Wait(50000);
    console.log("✅ Done");
    ...
    await wNativeRelayerContract.setCallerOk([vault.address], true, { gasLimit: '2000000' });
    Wait(50000);
    console.log("✅ Done")
